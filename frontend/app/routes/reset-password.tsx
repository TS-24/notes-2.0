...
export async function loader({ request }: Route.LoaderArgs) {
  // The token rides in the query string of the emailed link. No session check:
  // resetting from another device while still signed in on this one is allowed.
...
export async function action({ request }: Route.ActionArgs) {
  ...
  // Checked here as well as by the backend so the reader hears it before the
  // round trip, and hears the same thing either way — as register.tsx does.
  ...
  try {
    ...
    if (error instanceof ApiError) {
      // A 400 for a spent or expired link lands here.
