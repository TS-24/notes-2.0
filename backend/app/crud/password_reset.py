...
Only the SHA-256 of a token is ever passed in here. The token itself is shown
once to the superuser who issued it and lives nowhere else; see
`core/security.py::hash_reset_token`.
"""
...
def get_valid_by_hash(db: Session, token_hash: str) -> PasswordResetToken | None:
    ...
    attacker which part of a guess to keep.
    """
...
def invalidate_for_user(db: Session, user_id: int) -> int:
    ...
    never has more than one live link and a superseded link stops working the
    moment a new one is handed out. Does not commit.
    """
    result = db.execute(
...
def prune_expired(db: Session) -> int:
    ...
    one row per reset issued, forever.
    """
    result = db.execute(
