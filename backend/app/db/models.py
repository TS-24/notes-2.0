...
class User(Base):
    ...
    # refused by get_current_user, so a reset also ends every session the
    # account already had — which is the point when the reason for resetting is
    # that somebody else is in it. NULL means the password has never been
    # reset, which is every account until it goes through the flow.
...
class PasswordResetToken(Base):
    ...
    Only the SHA-256 of the token is stored. The token itself is shown once, to
    the superuser who issued it, and then exists only in the link they pass on —
    the same arrangement a password has with its hash, and for the same reason:
    a link that could be re-read from the database would make a database copy
    worth an account. That is why this is not listed back the way invite codes
    are; an invite is worth a new account bound to one address, a reset link is
    worth an existing one.

    `used_at` is both the record that a reset happened and the thing that stops
    the link working twice, which is the shape `InviteCode` uses for that job.
    ...
    id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True)
    user_id: Mapped[int] = mapped_column(nullable=False, index=True)
    # SHA-256 hex of the issued token: 64 characters exactly. Unique, so a
